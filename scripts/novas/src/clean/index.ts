...
import { colors, getPackages, rimraf, spinner } from '@vue3-ts-vite-template/node-utils'
...
async function runClean({ delLock = false, dirs = [], recursive }: CleanCommandOptions) {
...
async function clean({ delLock, dirs = [], recursive }: CleanCommandOptions) {
  ...
  await Promise.all(dirs.map((dir) => rimraf(join(process.cwd(), dir), { preserveRoot: true })))
}

function defineCleanCommand(cac: CAC) {
  cac
    .command('clean [dirs...]')
    .usage(`Delete all ['dist', 'node_modules', '.turbo'] directories under the project.`)
    ...
    .action(async (dirs, { delLock, recursive }) => await runClean({ delLock, dirs, recursive }))
}
